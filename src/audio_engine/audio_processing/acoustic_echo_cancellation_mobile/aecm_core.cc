...
// Forward declarations of NE10 functions
extern "C" {
...
AecmCore* WebRtcAecm_CreateCore() {
  ...
  aecm->fft_cfg_ = ne10_fft_alloc_r2c_float32(PART_LEN2);
  aecm->fft_tmp_in_ = new float[PART_LEN2];
  aecm->fft_tmp_out_ = new float[(PART_LEN + 1) * 2];
  if (aecm->fft_cfg_ == nullptr) {
    WebRtcAecm_FreeCore(aecm);
    return nullptr;
  }
...
void WebRtcAecm_FreeCore(AecmCore* aecm) {
  ...
  ne10_fft_destroy_r2c_float32(
      static_cast<ne10_fft_r2c_cfg_float32_t>(aecm->fft_cfg_));
  delete[] aecm->fft_tmp_in_;
  delete[] aecm->fft_tmp_out_;
