...
#include "audio_processing/acoustic_echo_cancellation_mobile/aecm_core.h"
#include "audio_processing/acoustic_echo_cancellation_mobile/aecm_defines.h"
...
void WindowAndFFT(AecmCore* aecm,
  ...
  // NE10 real FFT: int16 windowed time → float → r2c → extract complex.
  auto* cfg = static_cast<ne10_fft_r2c_cfg_float32_t>(aecm->fft_cfg_);
  for (i = 0; i < PART_LEN2; ++i)
    aecm->fft_tmp_in_[i] = static_cast<float>(fft[i]);
  ne10_fft_r2c_1d_float32_c(
      reinterpret_cast<ne10_fft_cpx_float32_t*>(aecm->fft_tmp_out_),
      aecm->fft_tmp_in_, cfg);

  // Extract complex values from NE10 output.
  // Scale by 1/PART_LEN2 to match Ooura fixed-point scaling (≈7 right-shifts).
  auto* fout = reinterpret_cast<ne10_fft_cpx_float32_t*>(aecm->fft_tmp_out_);
  const float fwd_scale = 1.f / static_cast<float>(PART_LEN);
  freq_signal[0].real =
      static_cast<int16_t>(fout[0].r * fwd_scale);
  freq_signal[0].imag = 0;
  for (i = 1; i < PART_LEN; ++i) {
    freq_signal[i].real =
        static_cast<int16_t>(fout[i].r * fwd_scale);
    freq_signal[i].imag =
        static_cast<int16_t>(-fout[i].i * fwd_scale);
  }
  freq_signal[PART_LEN].real =
      static_cast<int16_t>(fout[PART_LEN].r * fwd_scale);
...
void InverseFFTAndWindow(AecmCore* aecm,
  ...
  // NE10 inverse FFT: int16 complex → float → c2r → int16.
  auto* cfg = static_cast<ne10_fft_r2c_cfg_float32_t>(aecm->fft_cfg_);
  auto* fin = reinterpret_cast<ne10_fft_cpx_float32_t*>(aecm->fft_tmp_out_);
  fin[0].r = static_cast<float>(efw[0].real);
  fin[0].i = static_cast<float>(-efw[0].imag);
  for (i = 1; i < PART_LEN; ++i) {
    fin[i].r = static_cast<float>(efw[i].real);
    fin[i].i = static_cast<float>(-efw[i].imag);
  }
  fin[PART_LEN].r = static_cast<float>(efw[PART_LEN].real);
  fin[PART_LEN].i = static_cast<float>(-efw[PART_LEN].imag);

  ne10_fft_c2r_1d_float32_c(aecm->fft_tmp_in_, fin, cfg);

  // NE10 c2r is unscaled; Ooura ComplexIFFT applies ≈7 right-shifts.
  // Scale up by PART_LEN2 to compensate, then convert to int16.
  const float inv_scale = static_cast<float>(PART_LEN2);
  for (i = 0; i < PART_LEN2; ++i) {
    float val = aecm->fft_tmp_in_[i] * inv_scale;
    ifft_out[i] = static_cast<int16_t>(
        std::max(-32768.f, std::min(32767.f, val)));
  }

  // outCFFT equivalent: NE10 had no internal scaling, but we compensated
  // by multiplying by PART_LEN2 above, so set outCFFT = 0 to match.
  const int outCFFT = 0;
