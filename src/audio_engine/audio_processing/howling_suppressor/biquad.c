...
void sf_biquad_process_short(sf_biquad_state_st *state, int size, short *input,
	short *output)
{
	float * tmp_in = (float *)malloc(sizeof(float) * size);
	float * tmp_out = (float *)malloc(sizeof(float) * size);
	for (int i = 0; i < size; i++)
	{
		tmp_in[i] = (float)input[i] / (float)32768.0;
	}
	sf_biquad_process(state, size, tmp_in, tmp_out);
	for (int i = 0; i < size; i++)
	{
		if (tmp_out[i] > 0.999f)
			tmp_out[i] = 0.999f;
		if (tmp_out[i] < -0.999f)
			tmp_out[i] = -0.999f;
		output[i] = (short)(tmp_out[i] * 32767);
	}
	free(tmp_in);
	free(tmp_out);
}
