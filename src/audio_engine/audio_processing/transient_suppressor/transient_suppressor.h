...
// Fallback for FRIEND_TEST_ALL_PREFIXES when gtest is not available.
...
class TransientSuppressor {
 ...
 private:
  ...
  // NE10 FFT config and complex buffer (R2C/C2R out-of-place).
  ne10_fft_r2c_cfg_float32_t fft_cfg_;
  std::unique_ptr<ne10_fft_cpx_float32_t[]> fft_cplx_;

  std::unique_ptr<float[]> spectral_mean_;

  // Stores the real data for the fft.
