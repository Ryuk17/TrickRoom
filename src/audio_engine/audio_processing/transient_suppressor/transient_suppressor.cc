...
#include "audio_processing/transient_suppressor/common.h"
#include "audio_processing/transient_suppressor/transient_detector.h"
#include "signal_processing/windows_private.h"
...
TransientSuppressor::~TransientSuppressor() {
  if (fft_cfg_) {
    ne10_fft_destroy_r2c_float32(fft_cfg_);
    fft_cfg_ = NULL;
  }
}
...
int TransientSuppressor::Initialize(int sample_rate_hz,
  ...
  // Allocate NE10 R2C/C2R FFT config and complex buffer.
  if (fft_cfg_) {
    ne10_fft_destroy_r2c_float32(fft_cfg_);
    fft_cfg_ = NULL;
  }
  fft_cfg_ = ne10_fft_alloc_r2c_float32(static_cast<ne10_int32_t>(analysis_length_));
  fft_cplx_.reset(
      new ne10_fft_cpx_float32_t[complex_analysis_length_]());
...
void TransientSuppressor::Suppress(float* in_ptr,
  ...
  // NE10 R2C FFT: real input → natural-order complex output
  // {R[0], I[0], R[1], I[1], ..., R[n/2], I[n/2]} (no shuffle needed).
  ne10_fft_r2c_1d_float32_c(fft_cplx_.get(), fft_buffer_.get(), fft_cfg_);

  for (size_t i = 0; i < complex_analysis_length_; ++i) {
    magnitudes_[i] = ComplexMagnitude(fft_cplx_[i].r, fft_cplx_[i].i);
  }
  ...
  // Back to time domain.
  // NE10 C2R IFFT: natural-order complex input → real output.
  // NE10 applies 1/N internally, so 2.f * (1/N) == 2/N matches the
  // original rdft code's fft_scaling.
  ne10_fft_c2r_1d_float32_c(fft_buffer_.get(), fft_cplx_.get(), fft_cfg_);
  const float fft_scaling = 2.f;
...
void TransientSuppressor::UpdateKeypress(bool key_pressed) {
  ...
  if (keypress_counter_ > kIsTypingThreshold) {
    if (!suppression_enabled_) {
      LOG_INFO("[ts] Transient suppression is now enabled.");
    }
  ...
  if (detection_enabled_ &&
    ...
    if (suppression_enabled_) {
      LOG_INFO("[ts] Transient suppression is now disabled.");
    }
    detection_enabled_ = false;
    suppression_enabled_ = false;
    keypress_counter_ = 0;
...
// Attenuates by a certain factor every peak in the |fft_cplx_| that exceeds
...
void TransientSuppressor::HardRestoration(float* spectral_mean) {
  ...
  for (size_t i = 0; i < complex_analysis_length_; ++i) {
    if (magnitudes_[i] > spectral_mean[i] && magnitudes_[i] > 0) {
      ...
      fft_cplx_[i].r = (1 - detector_result) * fft_cplx_[i].r +
                       scaled_mean * cosf(phase);
      fft_cplx_[i].i = (1 - detector_result) * fft_cplx_[i].i +
...
void TransientSuppressor::SoftRestoration(float* spectral_mean) {
  ...
  for (size_t i = 0; i < complex_analysis_length_; ++i) {
    if (magnitudes_[i] > spectral_mean[i] && magnitudes_[i] > 0 &&
      ...
      fft_cplx_[i].r *= magnitude_ratio;
      fft_cplx_[i].i *= magnitude_ratio;
