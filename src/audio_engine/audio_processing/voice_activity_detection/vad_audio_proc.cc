...
#include "audio_processing/voice_activity_detection/vad_audio_proc.h"
...
#include "audio_processing/voice_activity_detection/common.h"
#include "audio_processing/voice_activity_detection/pitch_internal.h"
#include "audio_processing/voice_activity_detection/pole_zero_filter.h"
#include "audio_processing/voice_activity_detection/vad_audio_proc_internal.h"
#include "utils/checks.h"

// Use NE10 FFT directly
...
extern "C" {
#include "audio_processing/voice_activity_detection/isac/main/source/filter_functions.h"
#include "audio_processing/voice_activity_detection/isac/main/source/isac_vad.h"
#include "audio_processing/voice_activity_detection/isac/main/source/pitch_estimator.h"
#include "audio_processing/voice_activity_detection/isac/main/source/structs.h"
}
...
VadAudioProc::VadAudioProc()
  ...
  // Allocate NE10 FFT config and temp output buffer
  fft_cfg_ = ne10_fft_alloc_r2c_float32(kDftSize);
  fft_tmp_out_ = new float[(kDftSize / 2 + 1) * 2];
  // Initialize FFT with a warm-up call
  {
...
void VadAudioProc::FindFirstSpectralPeaks(double* f_peak,
  ...
  for (size_t i = 0; i < kNum10msSubframes; i++) {
    ...
    // Transform to frequency domain using NE10 real FFT.
    {
      auto* fout = reinterpret_cast<ne10_fft_cpx_float32_t*>(fft_tmp_out_);
      ne10_fft_r2c_1d_float32_c(
          fout, data,
          static_cast<ne10_fft_r2c_cfg_float32_t>(fft_cfg_));
      // Convert NE10 output to Ooura-compatible packed format in data[]
      const size_t ncfft = kDftSize / 2;
      data[0] = fout[0].r;
      data[1] = fout[ncfft].r;
      for (size_t i = 1; i < ncfft; ++i) {
        data[2 * i]     = fout[i].r;
        data[2 * i + 1] = fout[i].i;
      }
    }
