 ...
 * @LastEditors: Ryuk
 * @LastEditTime: 2026-08-16 10:05:53
 * @Description: First create
-->
# 功能
将`audio_processing`算法封装成统一的接口形式，接口相关定义在`src\audio_engine\interface`中。需要进行如下步骤。相对路径以git项目顶层为起点，当前路径为`src\audio_engine`。
...
- 封装后需要修改`src\audio_engine\CMakeLists.txt`编译使用封装接口的库，库名称格式为`libAE_xxx.a/so/dll`(比如libAE_AEC.a, xxx要大写)
